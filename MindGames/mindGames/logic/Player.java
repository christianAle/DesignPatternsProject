		...
		for (int runNum = 1; runNum <= 2; runNum++) {
			while (true) {
				if (runNum == 1) { // on first run ask for location to move from
					System.out.print(namePlayer + ", input your location to move from. (EX: E4)\n>> "); // prompt
				} else { // on second run ask pos to move to
					System.out.print(namePlayer + ", input you location to move to. (EX: E4)\n>> ");
				}
				String moveIn = scanner.nextLine().trim();

				// checking for blank spaces and nothing entered and no more than 2 characters
				// long
							...
							if ((y = convertCharNumtoNum(moveIn.charAt(1))) != -1) {
								...
								if (runNum == 1) {
									if (Board.square[y][x].getType() == "blank"
										...
										// returning an array full of -1's if the first location does not point to a
										// piece
